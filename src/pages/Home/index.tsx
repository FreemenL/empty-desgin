...
class Home extends Component<any, any> {
  ...
  renderChildren() {
    const Child = (
      <ChildRoute
        route={Routes.filter((route: any) => !mainRoute.includes(route.path))}
        type="child"
      />
    );
    const Result =
      location.hash === '#/home/home' ? (
        ...
        <div className="animated fadeIn emptyd-content">{Child}</div>
      );
    return Result;
  }
    ...
    return (
      <Layout className={contentClass}>
        ...
        <Layout>
          ...
          <Layout>
            {isHome ? null : <Ebeard menus={menuList} />}
            <Content id="content" className={contentWrapperClass}>
              {this.renderChildren()}
              <Footer className={contentFooterClass}>{footerText}</Footer>
            </Content>
