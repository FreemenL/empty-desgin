...
const data = [
  ...
  },
  {
    ...
    title: 'onClose',
    explain:
      '点击遮罩层 或 回退按钮 或 右上角叉或取消按钮的回调，children 的类型为React.ReactNode时 可在children组件的props中获取到 onClose 关闭抽屉',
    type: 'function(event)',
    default: '-',
  },
  {
...
class EdrawerDocuments extends Component<any, any> {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      !is(nextProps, this.props) ||
      this.state.visible !== nextState.visible ||
      this.state.groupVisible !== nextState.groupVisible
    ) {
      return true;
    }
    return false;
      ...
      <Edocument
        ...
        components={[
          ...
          },
          {
            component: (
              <section>
                <Button type="primary" onClick={this.handleGroupShow}>
                  {' '}
                  完美搭配{' '}
                </Button>
                  ...
                  {React.createElement(
                    ElistHoc(
                      ...
                      class Effrext extends Component {
                        render() {
                          return (
                            <span className="e-title-size-16-yellow">
                              这里的组件都可以在props中获取到params，用例场景:详情页
                            </span>
                          );
                        }
                      }
