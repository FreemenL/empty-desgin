...
export const FormConfig = function(this: any /*params*/) {
    ...
    searchPanel: {
      search: [
        ...
        },
        {
          ...
          type: 'Custom', //自定义类型
          label: '下拉选择',
          renderCustom: class extends Component<any, any> {
            constructor(props) {
              super(props);
        ...
        },
        {
          ...
          type: 'Custom', //自定义类型
          label: '班次标记',
          renderCustom: class InputColor extends Component<any, any> {
            constructor(props) {
              super(props);
              this.handleChange = this.handleChange.bind(this);
            }
            ...
            render() {
              return <EcolorPicker handleChange={this.handleChange} width="100%" />;
            }
          },
          antdFormItemOptions: {
            colon: false,
        ...
        },
        {
          ...
          antdOptions: {
            initialValue: [
              {
                url:
                  'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1542869766716&di=3f9a2a6f5c5b2c950fab91a7212c78ce&imgtype=0&src=http%3A%2F%2Ff.hiphotos.baidu.com%2Fimage%2Fpic%2Fitem%2Fd439b6003af33a8724e4b645cb5c10385243b5e0.jpg',
                fileId: 123,
              },
            ],
            rules: [
            ...
            Reflect.ownKeys(values).forEach(item => {
              if (
                (typeof values[item] !== 'number' &&
                  values[item].constructor &&
                  values[item].constructor.name == 'Moment') ||
                values[item].constructor.name == '_'
              ) {
  ...
  if (GetType(that) === 'undefined') {
    result.searchPanel.search.forEach((item, index) => {
      if (item.antdOptions && item.tag !== 'InputColor' && item.tag !== 'uploadImage') {
        delete item.antdOptions.initialValue;
      }
      if (index !== 0 && item.antdOptions) {
        item.antdOptions.initialValue = null;
      }
    });
  }
  return result;
