...
const patterns = [{
    name: "dev",
    run: () => run("cross-env NODE_ENV=development TS_NODE_PROJECT=\"config/tsconfig-for-webpack-config.json\" nodemon -e js,ts --watch build --watch package.json --watch config --exec \"webpack-dev-server  --hot --inline --display-error-details --mode development --config ./build/webpack.config.ts\""),
}, {
    name: "run",
